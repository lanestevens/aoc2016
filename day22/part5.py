...
def steps(grid, target, start, q=None, fname_template=None ):
    if q is None:
        q = [(grid, start, [])]
    while q:
        ...
        q = q[1:]
        if len(q) > 72000 and fname_template:
            i = 0
            while q:
                with open(fname_template.format(i), 'w') as f:
                    cPickle.dump({'target': target, 'q': q[:500]}, f)
                q = q[500:]
            return []
    return []
...
new_template = sys.argv[1] + '-{:04d}'
the_steps = steps(None, d['target'], None, q=d['q'], fname_template=new_template)
if the_steps:
    with open("solved-" + sys.argv[1], 'w') as f:
        f.write('{:d}'.format(len(the_steps)))
...
#print len(steps(grid, (0, 0), start_node(grid), fname_template='day22-{:08d}'))
